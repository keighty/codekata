class School
  attr_accessor :student_collection
  ...
  def students(grade)
    return [] unless student_collection[grade]
    student_collection[grade].sort
  end

  def add(name, grade)
    if grade_list = student_collection[grade]
      grade_list << name
    else
      student_collection[grade] = [name]
    end
  end

  def students_by_grade
    student_collection.sort.collect do |key, students|
      unless students(key).empty?
